    ...
    <div id='game<?php echo $match["game_id"];?>' class="<?php echo $session?($match["starter_id"] == $session["user_id"] or $match["player_id"] == $session["user_id"])?"mymatch":"publicmatch":"publicmatch"; ?> row" style="justify-content:space-between;">
            ...
            <?php else : ?>
                ...
                <?php if (!$session) : ?>
                    ...
                    <button onclick="login()" class="btn-primary">Join Match (<?php if ($match["starter_value"]>200) {echo floor($match["starter_value"]-($match["starter_value"]*0.05));} else {echo $match["starter_value"] - 10;}; ?> - <?php if ($match["starter_value"]>200) {echo floor($match["starter_value"]+($match["starter_value"]*0.05));} else {echo $match["starter_value"] + 10;}; ?>)</button>

                <?php elseif ($match["starter_id"] != $session["user_id"]) : ?>

                    <button onclick='joinMatch(<?php echo $match["game_id"] . "," . $match["starter_value"]; ?>)' class="btn-primary">Join Match (<?php if ($match["starter_value"]>200) {echo floor($match["starter_value"]-($match["starter_value"]*0.05));} else {echo $match["starter_value"] - 10;}; ?> - <?php if ($match["starter_value"]>200) {echo floor($match["starter_value"]+($match["starter_value"]*0.05));} else {echo $match["starter_value"] + 10;}; ?>)</button>
...
<div style="display:flex;flex-direction:column;text-align:center;padding-right:10px;">
    ...
    <?php if (!$match["end_date"]) : ?>
        ...
        <div style="color:cadetblue">(<?php if ($match["starter_value"]>200) {echo floor($match["starter_value"]-($match["starter_value"]*0.05));} else {echo $match["starter_value"] - 10;}; ?> - <?php if ($match["starter_value"]>200) {echo floor($match["starter_value"]+($match["starter_value"]*0.05));} else {echo $match["starter_value"] + 10;}; ?>)</div>
