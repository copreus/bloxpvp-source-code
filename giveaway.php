...
if ($_POST["type"] == "create") {
    ...
    if (file_exists('game_history/' . $session["user_id"]) == false) {
        jsonError("You should play a game for last 24 hours to join giveaway!");
    }
    if (time() - intval(file_get_contents('game_history/' . $session["user_id"])) > (24 * 60 * 60)) {
        jsonError("You should play a game for last 24 hours to join giveaway!");
    }
    ...
    ?>
        <div id='giveaway<?php echo $match["giveaway_id"]; ?>' class="row" style="justify-content:space-between;">
            ...
            <div style="display:flex;flex-direction:column;align-items:center;flex: 0 0 auto;">
                ...
                <span style="font-size:1.2em;color:lime;margin-top:-0.2em;" class="endtime">Ends in 30 Minutes</span>
            </div>
            <div style="display:flex;flex-direction:column;align-items:center;flex: 0 0 auto;">
