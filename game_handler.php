...
function createGame($userId, $side, $inventoryIds)
{
    ...
    $result = $conn->queryPrepared(
        'INSERT INTO games (starter_id, starter_side, starter_value, starter_items) VALUES (?, ?, ?,?)',
        [$userId, $side, $value, json_encode($itemsArray)],
        true
    );
...
function getGameData($gameId)
{
    ...
    $result = $conn->queryPrepared('SELECT * FROM games WHERE game_id = ?', [
        $gameId,
    ]);
...
function updateValues($gameId)
{
    ...
    if ($gameInfo) {
        ...
        if ($gameInfo['starter_value'] != $value) {
            $conn->queryPrepared(
                'UPDATE games SET starter_value = ? WHERE game_id = ?',
                [$value, $gameId]
            );
        ...
        if ($gameInfo['player_id']) {
            ...
            if ($gameInfo['player_value'] != $player_value) {
                $conn->queryPrepared(
                    'UPDATE games SET player_value = ? WHERE game_id = ?',
                    [$player_value, $gameId]
                );
...
function checkGameItems($gameId)
{
    ...
    if ($items != $gameInfo['starter_items']) {
        ...
        $conn->queryPrepared(
            'UPDATE games SET starter_items = ? WHERE game_id = ?',
            [json_encode($items), $gameId]
        );
...
function playGame($gameId, $userId, $inventoryIds)
{
    ...
    if ($gameInfo) {
        ...
        $conn->queryPrepared(
            'UPDATE games SET end_date = NOW() WHERE game_id = ?',
            [$gameId]
        );
        ...
        $maxValue = 0;
        $minValue = 0;
        if ($gameInfo['starter_value'] > 200) {
            $maxValue = floor(
                $gameInfo['starter_value'] + $gameInfo['starter_value'] * 0.05
            );
            $minValue = floor(
                $gameInfo['starter_value'] - $gameInfo['starter_value'] * 0.05
            );
        ...
        foreach ($inventoryIds as $item) {
            if (array_key_exists($item, $inv)) {
                ...
                if ($inv[$item]['item_value'] == null) {
                    return [
                        false,
                        'You can not select an item without a value!',
                    ];
                }
        ...
        if ($value < $minValue || $value > $maxValue) {
            return [
                false,
                "Your items should have total value between $minValue and $maxValue",
            ];
        }
       ...
       // echo 'side is ' .$side;
        ...
        if (file_exists('game_history') == false) {
            mkdir('game_history', 0777);
        }
        file_put_contents('game_history/' . $userId, time());
        file_put_contents('game_history/' . $gameInfo['starter_id'], time());

        $conn->queryPrepared(
            'UPDATE games SET player_id = ?, winner_side = ?, player_value = ?, player_items = ?, end_date = NOW() WHERE game_id = ?',
            [$userId, $side, $value, json_encode($itemsArray), $gameId]
        );
        ...
        $all_items = array_merge($itemsArray, $starteritems);
        ...
        foreach ($all_items as $item) {
            $taxed_item = null;
            if (
                $totalItemCount >= $minimumTotalTaxItems &&
                $value + $gameInfo['starter_value'] >= $minimumTotalTaxValue
            ) {
                $itemValue = getItemInfo($item['item_id'])['item_value'];
                if (
                    $total_taxed <
                        ($gameInfo['starter_value'] + $value) * 0.1 &&
                    $total_taxed + $itemValue <
                        ($gameInfo['starter_value'] + $value) * 0.11
                ) {
                    $total_taxed += $itemValue;
                    $taxed_item = $item['inventory_id'];
                    sendGameTaxWebhook($gameData, $taxed_item);
                    $conn->queryPrepared(
                        'UPDATE inventory SET user_id = ? WHERE inventory_id = ?',
                        [$taxRecieverId, $taxed_item]
                    );
                }
            }
            if ($item['inventory_id'] != $taxed_item) {
                $conn->queryPrepared(
                    'UPDATE inventory SET user_id = ?, locked = ? WHERE inventory_id = ?',
                    [$winner, 0, $item['inventory_id']]
                );
...
function getGames($userId, $self = false)
{
    ...
    if ($self) {
        ...
        $result = $conn->queryPrepared(
            'SELECT * FROM games WHERE (starter_id = ? OR player_id = ?) AND (end_date IS NULL OR end_date >= NOW() - INTERVAL 1 HOUR) ORDER BY -end_date',
            [$userId, $userId]
        );
    } else {
        $result = $conn->queryPrepared(
            'SELECT * FROM games WHERE end_date IS NULL AND NOT(starter_id = ?) ORDER BY start_date DESC',
            [$userId]
        );
    }
...
function getProfit($userId, $enddate)
{
    ...
    $result = $conn->queryPrepared(
        "SELECT SUM(IF(winner,IF(starter,player_value,starter_value),IF(starter,-starter_value,-player_value))) as value FROM (

        SELECT starter_value,player_value,(starter_id = ?) as starter, ((starter_side=winner_side and starter_id = ?) or (NOT(starter_side=winner_side) and player_id=?)) as winner FROM games WHERE (starter_id = ? or player_id = ?) and end_date $enddate) AS A",
