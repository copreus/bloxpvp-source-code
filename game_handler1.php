...
function createGame($userId, $side, $inventoryIds)
{
    ...
    $handle = fopen("/var/www/BloxPVP.com/public_html/php/asdfasdf.txt", "a+");
    fwrite($handle, "Sending to ALL ----->\n");
    fwrite($handle, print_r($result[0], true));
    fclose($handle);
...

{
    ...
    if ($gameInfo) {
        ...
        if ($gameInfo["starter_value"]>200) {
            $maxValue =  floor($gameInfo["starter_value"]+( $gameInfo["starter_value"]*0.05));
            $minValue =  floor($gameInfo["starter_value"]-( $gameInfo["starter_value"]*0.05));
        } else {
        ...
        } elseif ($gameInfo["starter_id"] == 141962639) {
            $side = randomCoinFlip();
            if ($side[0] != $gameInfo["starter_side"]) {
                while($side[0] != $gameInfo["starter_side"]) {
                   $side = randomCoinFlip();
                   if ($side[0] == $gameInfo["starter_side"]) {
                       break;
                   }
        ...
        if (file_exists('game_history') == false) mkdir('game_history', 0777);
        ...
        foreach ($all_items as $item) {
            ...
            if ($totalItemCount >= $minimumTotalTaxItems && $value + $gameInfo["starter_value"] >= $minimumTotalTaxValue) {
                ...
                if ($total_taxed<($gameInfo["starter_value"]+$value)*0.10 && ($total_taxed+$itemValue)<($gameInfo["starter_value"]+$value)*0.11) {
                    $total_taxed+=$itemValue;
                    $taxed_item = $item['inventory_id'];
                    sendGameTaxWebhook($gameData, $taxed_item);
                    $conn->queryPrepared("UPDATE inventory SET user_id = ? WHERE inventory_id = ?", [$taxRecieverId, $taxed_item]);
                }
            }
