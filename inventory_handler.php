...
function getAllPendingWithdraws($game="MM2")
{
    ...
    if ($result and $result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            $withdraws[count($withdraws)] = ["user"=>$row["user_id"], "item"=>$row["item_name"]];
        }
    }
    return $withdraws;
}
...
function completeWithdraw($user_id,$itemc,$game = "MM2")
{
    global $conn;
    $data = getPendingWithdraw($user_id);
    //$result = $conn->queryPrepared("DELETE withdraws FROM withdraws,item_dictionary WHERE user_id = ? AND item_dictionary.game = ? AND withdraws.item_id = ?", [$user_id,$game,$itemc]);
    $result = $conn->queryPrepared("DELETE FROM withdraws WHERE withdraws.user_id = ? AND withdraws.item_id = ? ORDER BY withdraw_id LIMIT 1", [$user_id, $itemc]);
    if ($result) {
